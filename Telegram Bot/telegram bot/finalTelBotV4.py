...
updater = Updater(
    "1907270097:AAE2edEXGUisLIx_uUW-g7mFAmSM2Yql6Os", use_context=True)
message = "Working..."
api_id = 0
api_hash = ''
session_id = 0
...
async def send_function(update: Update, context: CallbackContext):
    ...
    while status:
        for i in id_list_temp:
            try:
                await client.send_message(entity=i, message=message_temp)
...
def check_function(update: Update, context: CallbackContext):
    ...
    if (update.message.text == "/add" or update.message.text == "add"):
        ...
        update.message.reply_text("""1. Enter "/addall" to select all stored groups.\n2. Enter "/removeall" to remove all the selected groups.
                                      \n3. Enter 'Remove link1 link2...' to remove one or more groups from selected groups.
                                      \n4. To add new groups or select perticular groups, Enter Link.\n\nEnter "/done when done adding groups.""")
        return ADD
...
def add_function(update: Update, context: CallbackContext):
    ...
    if (text == "/addall" or text == "Addall"):
        ...
        update.message.reply_text("Selected groups : " + ", ".join(id_list))
        return ADD
        ...
        for i in l:
            if i == "remove":
                ...
                if i not in id_list:
                    update.message.reply_text(
                        "id : " + i + "\nNo such group link selected.")
                else:
                    id_list.remove(i)

        update.message.reply_text(
            "Groups removed.\nCurrent selected groups : " + ", ".join(id_list))
        return ADD
    elif ("http" in text):
        l = text.split("\n")
        print(l)
